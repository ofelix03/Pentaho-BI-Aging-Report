DROP FUNCTION IF EXISTS odoo14.corrected_aging_receivable_summary(
  CHARACTER VARYING, 
  INTEGER,
  DATE, 
  CHARACTER VARYING
...
CREATE OR REPLACE FUNCTION odoo14.corrected_aging_receivable_summary(
    ...
    SELECT trim(both ' ' FROM v_schema_name) INTO v_schema_name;
     ...
     v_query_string = $$
        SELECT partner_id, 
               ...
               sum(intv5) "intv5", 
               sum(balance) "balance"
       FROM odoo14.corrected_aging_receivable_detailed(
           '$$|| v_schema_name ||$$', 
           $$|| p_interval ||$$, 
           '$$|| p_date ||$$'::date, 
           '$$|| p_partner_ids ||$$'
...
COMMENT ON FUNCTION odoo14.corrected_aging_receivable_summary(CHARACTER VARYING, INTEGER, DATE, CHARACTER VARYING) IS $$
  This function provides summarized aging receivable with partner filtering and feature to change date interval.

  SIGNATURE
  ---------
  odoo14.corrected_aging_receivable_summary(
    IN p_schema CHARACTER VARYING,
    IN p_interval INTEGER,
    IN p_date DATE,
    IN p_partner_ids CHARACTER VARYING
  ...
  1. SELECT partner_id, partner_name, intv0, intv1, intv2, intv3, intv4, intv5, balance
       FROM odoo14.corrected_aging_receivable_summary('fdw_sagedistribution_14', 30, now()::date, '(0)');

  2. SELECT partner_id, partner_name, intv0, intv1, intv2, intv3, intv4, intv5, balance
       FROM odoo14.corrected_aging_receivable_summary('fdw_powerfuels', 30, '2019-10-31', '(2850, 2582, 2429, 2759, 2577, 2859, 2574, 2545, 2776, 2414)');
